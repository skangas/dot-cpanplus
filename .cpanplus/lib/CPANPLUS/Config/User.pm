...
#last changed: Sat Sep 12 04:03:44 2009 GMT
...
sub setup {
    ...
    } elsif ( $hostname == 'lenin' ) {
        $conf->set_conf( cpantest_mx => 'smtp.bredband.net' );
    } elsif ( $hostname == 'marx' ) {
        $conf->set_conf( cpantest_mx => 'smtp.bredband.net' );
    }
    ...
    $conf->set_conf( email => 'stefankangas@gmail.com' );
  ...
  },
  {
    ...
    $conf->set_conf( makemakerflags => 'INSTALL_BASE=/home/skangas/perl' );
    $conf->set_conf( md5 => 1 );
